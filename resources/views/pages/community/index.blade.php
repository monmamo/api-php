<x-guest-layout>
<h2>Community Rules</h2>

<h5>Be respectful.</h5>
<ul>
<li>Treat other members with respect and courtesy.</li>
<li>Harassment, hate speech, discrimination, personal attacks, or demeaning language towards others will not be tolerated. When offering criticism, it should be constructive and aimed at helping improve, not belittling or disparaging others.</li>
<li>Discussions about politics, religion or controversial or sensitive topics may be terminated without warning if staff deems that the discussion is a detriment to the community.</li>
</ul>

<h5>Observe decorum.</h5>
<ul>
<li>Monsters Masters & Mobsters expects community members to speak and behave in a manner behooving adulthood.</li>
<li>Members are prohibited from posting certain content matter as listed below.</li>
<li>Members may use profanity and adult speech, but the community staff reserve the right to warn about excessive profanity or inappropriate speech and take appropriate action (including suspensions and bans) if such behavior persists.</li>
<li>Abbreviated, censored or coded profanity will be treated the same as explicit profanity.</li>
<li>While Monsters Masters & Mobsters is generally intended for adults, some situations and circumstances may be opened to younger audiences. In these cases, the staff reserve the right to impose additional or stricter rules on content and speech.</li>
</ul>

<h5>Stay on-topic.</h5>
<ul>
<li>Posts and discussions should be relevant to Monsters Masters & Mobsters and the particular topic or focus of the channel in which they are posted.</li>
</ul>

<h5>Respect and protect the privacy of others.</h5>
<ul>
<li>Do not share personal information about others without their consent. Sharing your own personal information is strongly discouraged. Staff may remove personal information of any sort, even if belonging to the person who posted it, without notice.</li>
</ul>

<h5>Give proper credit.</h5>
<ul>
<li>When sharing information or content from other sources, properly credit the original source.</li>
</ul>

<h5>No solicitations.</h5>

<ul>
<li>Community members are advised not to use the community to conduct sales or trades with other community members. Instead we encourage members who which to buy, sell or trade with others to use established marketplaces with safeguards for both buyer and seller.</li>
</ul>

<h5>Do not spam.</h5>
<ul>
<li>Do not send a lot of small messages right after each other. These disrupt the chat and make it hard to scroll through the server.</li>
<li>Please keep your messages at least 5 words long while chatting.</li>
<li>Please allow others to respond before making another post. (If at least five minutes post without a reply, you may make another relevant post.)</li>
</ul>

<h5>No offensive names and profile pictures.</h5>
<ul>
<li>These rules apply equally to offensive names and profile pictures.</li>
<li>Members who use offensive or inappropriate user names or profile pictures will be subject to immediate ban without warning.</li>
<li>Do not join voice chat channels without permission of the people already in there.</li>
<li>If you see that they have a free spot it is alright to join and ask whether they have an open spot, but leave if your presence is not wanted by whoever was there first.</li>
</ul>

<h5>Avoid false or misleading information.</h5>
<ul>
<li>Make sure to fact-check any information you share and only share reliable sources. Spreading false or misleading information can be harmful and undermines the trust and integrity of the community.Staff may remove any content that is intentionally misleading or falsely represents the actions, opinions and beliefs of any person.</li>
</ul>

<h5>Do not impersonate other users, staff members, or any other individuals or organizations.</h5>
<ul>
<li>This includes using similar usernames, profile pictures, or display names.</li>
</ul>

<h5>No excessive self-promotion.</h5>
<ul>
<li>Share your work in moderation, only in designated channels, and only as apropos to the topic at hand. Staff may remove any self-promotion that they deem inappropriate or excessive. Posting for the sole purpose of inappropriate self-promotion may result in an immediate ban.</li>
<li>Do not abuse server bots.</li>
<li>Only use server bots for their intended purposes and do not spam or misuse their commands.</li>
</ul>

<h5>Do not abuse emojis, capital letters or punctuation.</h5>
<ul>
<li>This can be disruptive and make the chat difficult to follow.</li>
<li>Individual posts may contain no more than three (3) emojis unless the emojis are necessary or prudent for conveying meaning and emotion.</li>
<li>The following emojis (and equivalents in other skin tones) are prohibited and may not be used in messages or as responses: 🖕</li>
<li>Emoji may not be used as replacements for profanity or innuendo.</li>
</ul>

<h5>Do not encourage, glorify, or engage in self-destructive behavior or self-harm.</h5>
<ul>
<li>This includes sharing graphic content related to self-harm or discussing personal experiences in a way that may trigger others.</li>
</ul>

<h2>Prohibited Content</h2>
<ul>
<li>Images or videos showing or depicting exposure of human genitalia.</li>

<li>Graphic depictions of violence.</li>

<li>Promotions of products or services related to sex.</li>

<li>Discussion, promotion or engagement in any activity that is expressly prohibited by the laws of the United States and/or the country hosting the particular forum or venue.</li>

<li>Content that demeans or dehumanizes any human being or group of persons, particularly (but not limited to) biological sex, physical ability or lack thereof, mental health, neurodivergence, body shape or form, sexual orientation or preference, gender identity or dysphoria, ethnic heritage or identity, religious identity or lack thereof.</li>

<li>Profanity or vulgar talk that is deemed to be excessive (after a moderator has given a warning of such).</li>

<li>Personal and/or information of any member that the member has not shared publicly in the community, including (but not limited to) full legal name; any nickname not used in the forum or venue; employer; address, city, jurisdiction or country of residence or employment; personal telephone numbers and email addresses; names of immediate family, friends or associates.</li>

<li>Financial or banking information, such as credit card numbers or bank account information</li>

<li>Any attempt to solicit personal information from members for any purpose other than direct communication and collaboration.</li>

<li>Solicitation or begging for money, items, or services.</li>

<li>Any solicitation of sales that is not an approved advertisement.</li>

<li>Any political or religious solicitation, including solicitation of votes for particular candidates or measures; promotion of political parties; and religious proselytizing. (Respectful explanations of political views and religious beliefs are allowed, as long as such explanations do not segue into solicitation.)</li>

<li>Any threat of bodily harm or disclosure of personal information against any member.</li>

<li>Links to illegal or pirated content, such as copyrighted material, stolen software, and keys or serial numbers for software or games that have been obtained illegally.</li>

<li>Hacks, cheats, or other unauthorized tools or services.</li>

<li>Viruses or malicious software.</li>

<li>Counterfeit or fraudulent items, such as fake IDs or fraudulent financial documents in this server.</li>

<li>Recruiting or promotion of other servers or communities without prior approval from the server staff.</li>
</ul>

<h2>Enforcement</h2>
<ul>
<li>Enforcement of these rules helps maintain a positive and productive environment for all community members. Violations may result in content removal, warnings, or bans from the community at the moderators' discretion.</li>

<li>Moderators are allowed to remove, without warning, any content that threatens the safety of any member or may subject this community or Monsters Masters & Mobsters LLC to legal action.</li>

<li>Monsters Masters & Mobsters LLC reserves the right to pursue appropriate legal action to seek recompense from any person or entity whose actions subject the company to legal action.</li>
</ul>
</x-guest-layout>
